...
void DrawSettings (System sys) {//: master volume , music volume , effects volume
	...
	//Exit
	DrawText("Exit",(screenWidth/2) - (MeasureText("Exit",buttonFont)/2)-2,428+shiftDown,buttonFont,BLACK);
	DrawText("Exit",(screenWidth/2) - (MeasureText("Exit",buttonFont)/2),430+shiftDown,buttonFont,BLACK);
	DrawText("Exit",(screenWidth/2) - (MeasureText("Exit",buttonFont)/2)+2,432+shiftDown,buttonFont,(sys.choice==40+settingsOpt)?RED:WHITE);
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (sys.choice<10) {//Lobby ------------------------------------
			...
			if (IsKeyPressed(KEY_ENTER) && sys.choice==40+settingsOpt) sys.choice=1;//exit
